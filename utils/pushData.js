...
async function main({organisationName="A", organisationNumber=1, userId, data}) {
    ...
    try {
        ...
        // addRow - org, data
        const result = await contract.evaluateTransaction('addRow', organisationName, JSON.stringify({...data, vehicleNo: userId}));
        console.log(`Transaction has been evaluated, result is: ${result.toString()}`);

        // Disconnect from the gateway.
        await gateway.disconnect();

        return Promise.resolve(`Transaction has been evaluated, result is: ${result.toString()}`);
